...
#____________________________subquery
...
def films_by_cast_size
  ...
  execute(<<-SQL)
    SELECT
      title, COUNT(DISTINCT castings.actor_id)
    FROM 
      movies 
    JOIN 
      castings ON movies.id = castings.movie_id 
    ...
    GROUP BY 
      movies.id
    ORDER BY 
      COUNT(Distinct castings.actor_id) desc, movies.title asc; 
  SQL
end
